...
class IQGeoCDIF:
        ...
        if self.src_lyr in layers:
            ...
            if self.input_gdb.endswith(".gdb"):
                ...
                if os.path.exists(sqlite_db):
                    ...
                    util_services.UTILServices.export_gdb_to_sqlite(self.input_gdb,sqlite_db,['SPAN','SPAN_UNIT','SPAN_SPAN_UNIT','EQUIPMENT','SLOT','PLUGIN','PORT','SLOT_PLUGIN','CHASSIS'])
    ...
    def g_shortest_path(self, G, source_node, destination_node):
        try:
            path = nx.shortest_path(G, source=source_node, target=destination_node, weight='weight')
            return {"path": path}
        except Exception as e:
            return {"error": str(e)}
                ...
                for asso_span_row in associated_span:
                    ...
                    for fld in self.reference_fields:
                        ...
                        if fld['Field_Name']=="in_structure":
                            ...
                            if not match.empty:
                                ...
                                # fld_val=str_name
                                frm_str_obj_id = ""
                                self.parent_missing.append([row[uqfld],"housing7",row[self.structure_right_clm + "_x"],row[self.structure_right_clm + "_y"]])
                        ...
                        if fld['Field_Name']=="out_structure":
                            ...
                            if not match.empty:
                                ...
                                # fld_val=str_name
                                to_str_obj_id = ""
                                self.parent_missing.append([row[uqfld],"housing8",row[self.structure_right_clm + "_x"],row[self.structure_right_clm + "_y"]])
                 ...
                 # Read Route CSV for Housing reference
                    ...
                    if not route_match.empty:
                        ...
                        self.parent_missing.append([row[uqfld],"housing1",hoursing_route_name, ""])
                    ########################
                    csv_row[1]=geom_wkb
                ...
                for fld in self.reference_fields:
                    ...
                    if fld['Field_Name']=="in_structure":
                        ...
                        if not match.empty:
                            ...
                            # fld_val=str_name
                            frm_str_obj_id = ""
                            self.parent_missing.append([row[uqfld],"housing2",row[self.in_structure_right_clm + "_x"],row[self.in_structure_right_clm + "_y"]])
                    ...
                    if fld['Field_Name']=="out_structure":
                        ...
                        if not match.empty:
                            ...
                            # fld_val=str_name
                            to_str_obj_id = ""
                            self.parent_missing.append([row[uqfld],"housing3",row[self.out_structure_right_clm + "_x"],row[self.out_structure_right_clm + "_y"]])
    ...
    # Read Route CSV for Housing reference
                ...
                if not route_match.empty:
                    ...
                    housing_route_id=hoursing_route_name
                    self.parent_missing.append([row[uqfld],"housing4",hoursing_route_name, ""])
                ...
                if len(conduit_gdf)>0:
                    ...
                    if "error" in data.keys():
                        ...
                        if non_exist == '' and data.get('error') == 'No Link between nodes':
                            path_name = self.generate_code(route_df)
                            ...
                            type_name = 'MESSENGER' if 'pole' in row['FROM_STRUCTURE_NAME'] or 'pole' in row['TO_STRUCTURE_NAME'] else 'BURIED'
                            new_row = {'id': '', 'path': dumps(geom, hex=True) , 'in_structure': row['FROM_STRUCTURE_NAME'],
                                        'out_structure': row['TO_STRUCTURE_NAME'], 'name': path_name,
                            ...
                            df_route_ref = pd.concat([df_route_ref, pd.DataFrame([{'id': '','name':path_name, 
                                                                               'in_structure': row['FROM_STRUCTURE_NAME'],
                                                                               'out_structure': row['TO_STRUCTURE_NAME']
                                                                               }])], ignore_index=True)
                    if "error" in data.keys():
                        if non_exist == "":
                            non_exist = data.get("error")
                        self.parent_missing.append([row['SPAN_NAME'],"Unable to get route for conduit",  Missing_conduit[1] , Missing_conduit[2], '', '', non_exist])
                    else:
                        ...
                        if length_of_all_parts<length_of_org_conduit*20:
                            ...
                            for route_name in data['path_name']:
                                ...
                                if not route_match.empty:
                                    ...
                                    housing_route_id=hoursing_route_name
                                    self.parent_missing.append([row[uqfld],"housing5",hoursing_route_name, ""])
                                    route_in_str=""
                                    route_out_str="" 
                            ...
                            self.shortest_route_exception_list.append([length_of_org_conduit*2, length_of_all_parts])
                            self.parent_missing.append([row['SPAN_NAME'],"Shortest route not found", 'Identified route is 50 percent longer then orginal counduit route ' ,data['path_name'], f"{length_of_org_conduit:.8f}", f"{length_of_all_parts:.8f}"])
            ...
            if (filtered_cnt>0):
                ...
                for index, row in filtered_df.iterrows():
                    ...
                    if not route_match.empty:
                        ...
                        self.parent_missing.append([formation_span_name,"housing6",formation_span_name, ""])
                        route_in_str=""
                        route_out_str=""
...
# account_code  st_std_desc
    ...
    def generate_code(self, df):
        while True:
            num = random.randint(10000000, 99999999)  # ensures 6-digit number
            code = f"OSP:COND::{num}"
            if code not in list(df['name']):
                return code   

    def get_shortest_path_edges(self,G, df, source_node, destination_node, ivs, vintage_yr):
        # if source_node == 'cabinet/2437' and destination_node == 'st_struct_pedestal/68396':
        # if source_node == 'st_struct_pedestal/67441' and destination_node == 'st_struct_pedestal/70422':
        ...
        if len(return_edges) == 1:
            ...
            try:
                ...
                self.error_count += 1   
                val = str(e)+" - Due to vintage year mismatch"
                raise Exception(val)
            ...
            if len(filter_by_path_name)>1:
                ...
                if all_path:
                    ...
                    raise Exception("No Link between nodes")
   ...
   # def FinalizeCDIF(self):
    ...
    def FinalizeCDIF(self):
        ...
        print(self.shortest_route_exception_list)
